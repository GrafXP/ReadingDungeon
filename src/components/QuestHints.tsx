...
import type { QuestViewDefinition, WorldDefinition } from '../domain/content'
import type { GameSave } from '../domain/game'
import type { GameAction } from '../engine/actions'
import { getHintLevel, getHintTexts } from '../engine/hints'

export function QuestHints({ game, quest, world, onAction }: { game: GameSave; quest: QuestViewDefinition; world: WorldDefinition; onAction: (action: GameAction) => void }) {
  const level = getHintLevel(game, quest)
  const texts = getHintTexts(quest, world.journal?.hintSteps)
  ...
  return (
    <details className="hint-details" onToggle={(event) => {
      ...
      <summary>Hinweis öffnen{level > 0 ? ` (${level} von 3)` : ''}</summary>
