...
function prologueQuests(save: GameSave): QuestViewDefinition[] {
  return [
    ...
    quest('prolog_fest', 'Folge Kunos Schatten', 'Gehe zum Festplatz und beruhige den Schattenzipfel.', hasFlag(save, 'schattenzipfel_beruhigt'), 'Lies seinen nächsten Zug. Licht hilft gegen Schatten.', ['sm_festplatz']),
...
function nearQuests(save: GameSave): QuestViewDefinition[] {
  return [
    ...
    quest('ww_arbor', 'Arbors Schatten zurückrufen', 'Öffne den Waldweg und hilf Arbor im Heiligtum.', hasFlag(save, 'arbors_schatten_zurueck'), 'Rüste das Astbeil aus. Kapphieb stoppt die Ladung.', ['ww_wurzelheiligtum']),
    ...
    quest('sk_marea', 'Mareas Schatten zurückrufen', 'Öffne das Muscheltor und hilf Marea im Tempel.', hasFlag(save, 'mareas_schatten_zurueck'), 'Schwallstoss macht den Schatten nass. Blitz trifft dann stärker.', ['sk_gezeitentempel']),
    ...
    quest('dh_voltaro', 'Voltaros Schatten zurückrufen', 'Stoppe den Kupferläufer und hilf Voltaro am Turm.', hasFlag(save, 'voltaros_schatten_zurueck'), 'Verteidige den Sturzflug. Kurzschluss stoppt die Ladung.', ['dh_gewitterturm'])
  ]
}
...
function outerQuests(save: GameSave): QuestViewDefinition[] {
  return [
    ...
    quest('fi_mantel', 'Den Feuermantel nähen', 'Sammle zwei Ofenfasern und eine Glutschale.', hasItem(save, 'item_armor_feuermantel'), 'Die Fasern wachsen am Hang. Nimas Zange erreicht die Schale.', ['fi_ofenfaserhang', 'fi_glutschalenfeld', 'fi_suppenkueche']),
    ...
    quest('fi_vogel', 'Den Feuervogel retten', 'Kühle den Ofenring und löse den Aschenbrüter.', hasFlag(save, 'feuervogel_gerettet'), 'Trage den Feuermantel. Stelle Alvas Klinge auf Eis.', ['fi_kuehlrinne', 'fi_aschennest_vorraum']),
    ...
    quest('lm_fuchs', 'Den Laternenfuchs retten', 'Lege den Lichtweg und löse die Nachtlaterne.', hasFlag(save, 'laternenfuchs_gerettet'), 'Trage den Umhang. Der Laternenstab hält Verstecktes sichtbar.', ['lm_nebelsteg', 'lm_nachtpfad'])
  ]
}
...
function finalQuests(save: GameSave): QuestViewDefinition[] {
  return [
    ...
    quest('rn_panzer', 'Den Wandelpanzer durchqueren', 'Lies Feuer, Eis und Blitz. Öffne den Schild mit Licht.', won(save, 'enc_rn_wandelpanzer'), 'Alvas Klinge kann am Rastplatz auf Licht gestellt werden.', ['rn_sternentreppe']),
    quest('rn_echo', 'Kunos Wahrheit hören', 'Hilf Kuno in der Halle der Echos.', won(save, 'enc_rn_echozwilling'), 'Die beiden Stimmen greifen abwechselnd an.', ['rn_halle_der_echos']),
    quest('rn_raugrim', 'Raugrims Fäden trennen', 'Öffne Raugrims vier Formen.', hasFlag(save, 'raugrim_faeden_getrennt'), 'Licht löst die Fäden. Verteidige schwere Züge.', ['rn_weltenkammer']),
...
function getMainGoal(save: GameSave) {
  ...
  if (outerFlags.every((value) => hasFlag(save, value))) return { title: 'Hilf Kuno im Reich der Nacht', description: 'Raste am Rand der Nacht. Lies jeden angekündigten Zug und trenne Raugrims Fäden.' }
  ...
  if (nearFlags.every((value) => hasFlag(save, value))) return { title: 'Öffne das Fernwegtor', description: 'Folge den geretteten Wächtern in die Verborgenen Wege und nimm Alvas Klinge.' }
...
export const talora2RuleCards: RuleCardDefinition[] = [
  ...
  { id: 'regel_verteidigen', title: 'Verteidigen', description: 'Ein schwerer angekündigter Zug wird ganz abgewehrt. Danach ist oft ein Treffer frei.', requirement: { kind: 'clue', clueId: 'regel_verteidigen' } },
  { id: 'regel_flucht', title: 'Flucht und Rettung', description: 'Du behältst alle Funde. Flucht und Rettung bringen dich zum letzten Rastplatz.', requirement: { kind: 'clue', clueId: 'regel_flucht' } },
...
export const talora2StoryBeats: NonNullable<WorldDefinition['storyBeats']> = [
  ...
  { id: 'raugrim_faeden', requirement: flag('raugrim_faeden_getrennt'), text: 'Raugrim ist gefallen, doch die Schatten sind noch nicht frei. Kuno tritt zu dir. «Ein Name und eine gute Erinnerung. Das ist stärker als sein Flüstern.»' },
